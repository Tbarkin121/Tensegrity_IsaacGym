...
asset_options.fix_base_link = False
support_asset = gym.load_asset(sim, asset_root, asset_file, asset_options)
...
cam_pos = gymapi.Vec3(0.5, 0.5, 0.1)
...
connection_matrix = torch.zeros((num_bodies, len(cfg["tensegrityParams"]["connections"])))
connection_list = []
idx = 0
...
connection_list = torch.tensor(connection_list) # need to be type long to be used as indicies
...
sensors_per_env = 12
vec_sensor_tensor = gymtorch.wrap_tensor(sensor_tensor).view(num_envs, sensors_per_env * 6)
# tensebot_pos = root_states.view(num_envs, num_actors, 13)[..., 0:6, 0:3] #num_envs, num_actors, 13 (pos,ori,Lvel,Avel)
foot_forces = vec_sensor_tensor.view(num_envs, num_actors*2, 6)[..., 0:3] #num_envs, num_actors, 13 (pos,ori,Lvel,Avel)
foot_torques = vec_sensor_tensor.view(num_envs, num_actors*2, 6)[..., 3:6] #num_envs, num_actors, 13 (pos,ori,Lvel,Avel)
